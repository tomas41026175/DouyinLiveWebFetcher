...
PIDS=()
cleanup(){ echo ""; c_say "關閉所有服務中…"; rm -f "$CHAT_URL_FILE"; for p in "${PIDS[@]:-}"; do kill "$p" 2>/dev/null || true; done; }
trap cleanup EXIT INT TERM

( cd "$ROOT" && DY_NO_AUTOCLOSE=1 DY_NO_BROWSER=1 "$VPY" web_danmaku.py >/dev/null 2>&1 ) &
PIDS+=($!)
( cd "$CHATROOM" && PORT=$CHAT_PORT DANMAKU_URL="http://127.0.0.1:$DANMAKU_PORT" DANMAKU_PASSWORD="$PASSWORD" node server.js >/dev/null 2>&1 ) &
PIDS+=($!)
sleep 3
